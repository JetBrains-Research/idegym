...
3. Install the `idegym` Helm release (the bundled PostgreSQL subchart provisions its own Secret)
...
# Uninstall the Helm release after tests (preserves namespace and PVCs)
uv run pytest -m e2e --uninstall-release
```
...
| `--delete-namespace`         | Delete `idegym-local` namespace in test teardown                             |
| `--uninstall-release`        | Uninstall the `idegym` Helm release in test teardown (keeps the namespace)   |
...
2. Install the chart via `helm upgrade --install -f e2e-tests/config/values.yaml charts/idegym`
