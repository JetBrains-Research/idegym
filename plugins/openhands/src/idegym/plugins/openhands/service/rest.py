...
def build_rest_router(runtime: ToolRuntime) -> APIRouter:
    ...
    # -- terminal lifecycle -----------------------------
    # Registered only when the terminal tool is enabled: a disabled terminal must not expose
    # lifecycle mutations. The runtime facade re-checks the policy on every call (defense in depth).
    ...
    if runtime.terminal_enabled():
        ...
        async def create_terminal(request: TerminalCreateRequest) -> Any:
            try:
                return await runtime.terminal_create(request)
            except ServiceError as exc:
                return _error_response(exc)
        ...
        async def get_terminal(terminal_id: str) -> Any:
            try:
                return runtime.terminal_get(terminal_id)
            except ServiceError as exc:
                return _error_response(exc)
        ...
        async def terminal_execute(terminal_id: str, request: TerminalExecuteRequest) -> Any:
            try:
                return await runtime.terminal_execute(
                    terminal_id,
                    request.command,
        ...
        async def terminal_input(terminal_id: str, request: TerminalInputRequest) -> Any:
            try:
                return await runtime.terminal_input(terminal_id, request.text, timeout=request.timeout)
            except ServiceError as exc:
                return _error_response(exc)
        ...
        async def terminal_poll(terminal_id: str, request: TerminalPollRequest) -> Any:
            try:
                return await runtime.terminal_poll(terminal_id, timeout=request.timeout)
            except ServiceError as exc:
                return _error_response(exc)
        ...
        async def terminal_interrupt(terminal_id: str) -> Any:
            try:
                return await runtime.terminal_interrupt(terminal_id)
            except ServiceError as exc:
                return _error_response(exc)
        ...
        async def terminal_reset(terminal_id: str) -> Any:
            try:
                return await runtime.terminal_reset(terminal_id)
            except ServiceError as exc:
                return _error_response(exc)
        ...
        async def terminal_capture(terminal_id: str) -> Any:
            try:
                return {"terminal_id": terminal_id, "content": await runtime.terminal_capture(terminal_id)}
            except ServiceError as exc:
                return _error_response(exc)
        ...
        async def terminal_close(terminal_id: str) -> Any:
            try:
                await runtime.terminal_close(terminal_id)
            except ServiceError as exc:
                return _error_response(exc)
        ...
        async def terminals_reset_all() -> Any:
            try:
                count = await runtime.terminal_reset_all()
            except ServiceError as exc:
                return _error_response(exc)
