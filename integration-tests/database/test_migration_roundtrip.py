...
# Same for 004, which persists build contexts.
COLUMNS_ADDED_BY_004 = {
    "job_statuses": {"build_context"},
}

# Same for 005, which adds the keepalive hold.
COLUMNS_ADDED_BY_005 = {
...
async def test_round_trip_through_every_revision(manager: MigrationManager):
    ...
    assert chain == ["001", "002", "003", "004", "005"], "update this test's per-revision expectations"
    ...
    for added in (COLUMNS_ADDED_BY_003, COLUMNS_ADDED_BY_004, COLUMNS_ADDED_BY_005):
...
async def test_downgrade_needs_explicit_approval(manager: MigrationManager):
    ...
    expected = COLUMNS_ADDED_BY_003["servers"] | COLUMNS_ADDED_BY_005["servers"]
