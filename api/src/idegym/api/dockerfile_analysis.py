...
# ``<<EOF``, ``<<-EOF``, ``<<'EOF'``, ``<<"EOF"``. The backreference keeps the closing quote
# matched to the opening one, and requiring a word character after the optional quote is what
# stops a shell here-string (``<<<``) from matching.
...
def logical_lines(content: str) -> list[LogicalLine]:
    ...
    while index < len(physical):
        ...
        end = _absorb_heredocs(physical, text, min(index, len(physical) - 1))
        result.append(LogicalLine(start=start, end=end, text=text))
        index = end + 1

    return result


def stages(content: str) -> list[Stage]:
...
def copy_add_sources(content: str) -> list[CopySource]:
    ...
    for line in logical_lines(content):
        ...
        if _HEREDOC_RE.search(line.text):
...
def buildkit_only_features(content: str) -> list[BuildKitFeature]:
    ...
    ``BUILDKIT_SYNTAX`` (or its own ``# syntax=``) to build on Cloud Build.
    """
...
def _absorb_heredocs(physical: list[str], text: str, end: int) -> int:
    """Extend ``end`` past the bodies of any heredocs the instruction opens.

    A heredoc whose terminator never appears is treated as a false positive rather than swallowing
    the rest of the file — ``RUN echo "a << b"`` matches the pattern but opens nothing.
    """
