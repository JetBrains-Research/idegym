...
def test_pycharm_render_installs_open_project_plugin_when_project_in_ctx():
    ...
    # The shared start-ide entrypoint + supervisord program are COPY'd from plugin-utils.
    assert "COPY plugins/plugin-utils/scripts/start-ide.sh /usr/local/bin/start-ide" in fragment
    assert "COPY plugins/plugin-utils/scripts/supervisord-ide.conf /etc/supervisor/conf.d/ide.conf" in fragment
...
def test_idea_headless_by_default():
    ...
    assert "'-Djava.awt.headless=true'" in fragment
    assert "'-Didea.trust.all.projects=true'" in fragment
...
def test_idea_render_virtual_display_installs_xvfb():
    ...
    # (The quoted printf arg only appears in headless builds; the header comment mentions the
    # flag in prose, so match the quoted form to distinguish the two.)
    assert "'-Djava.awt.headless=true'" not in fragment
...
@mark.parametrize(
    ("plugin", "vmoptions"),
    [
        param(Idea(), ("idea64.vmoptions", "idea.vmoptions"), id="idea"),
        param(PyCharm(), ("pycharm64.vmoptions", "pycharm.vmoptions"), id="pycharm"),
    ],
)
def test_ide_vmoptions_appended_one_flag_per_line(plugin, vmoptions):
    # Each flag is a separate quoted `printf '%s\n'` argument, not one giant embedded-newline
    # printf line, so the list stays readable and reviewable.
    fragment = plugin.render(BuildContext(base="debian:bookworm-slim"))
    for name in vmoptions:
        assert name in fragment
    assert "printf '%s\\n'" in fragment
    assert "'-XX:ErrorFile=/tmp/jvm-crash.log' \\" in fragment


def test_idea_and_pycharm_share_jetbrains_ide_base():
    # The two plugins are thin subclasses of the shared JetBrainsIdePlugin base in plugin_utils.
    from idegym.plugins.plugin_utils.ide_plugin import JetBrainsIdePlugin

    assert issubclass(Idea, JetBrainsIdePlugin)
    assert issubclass(PyCharm, JetBrainsIdePlugin)


@mark.parametrize("plugin", [param(Idea(headless=False), id="idea"), param(PyCharm(), id="pycharm")])
def test_generated_ide_settings_files_are_well_formed_xml(plugin, tmp_path):
    # The IDE settings XML is generated by shared, XML-validated Python helpers. Execute the
    # config-writing RUNs from the rendered Dockerfile and parse every .xml they produce, proving the
    # emitted Dockerfile writes only well-formed XML (not just that the source module validates).
    from xml.etree import ElementTree

    ctx = BuildContext(base="debian:bookworm-slim").with_extra("idegym.has_project", True)
    joined = plugin.render(ctx).replace("\\\n", " ")
    ran = 0
    for line in joined.split("\n"):
        if line.startswith("RUN ") and "printf" in line and "vmoptions" not in line and "ide-config" in line:
            cmd = line[4:].replace("/tmp/ide-config", str(tmp_path)).replace("/root/.local", str(tmp_path / "dotlocal"))
            subprocess.run(["sh", "-c", cmd], check=True, capture_output=True)
            ran += 1
    assert ran  # at least one config RUN was exercised
    xml_files = list(tmp_path.rglob("*.xml"))
    assert {p.name for p in xml_files} >= {"mcpServer.xml", "trusted-paths.xml", "updates.xml"}
    for path in xml_files:
        ElementTree.fromstring(path.read_text())  # raises on malformed XML


def test_ide_config_helper_rejects_malformed_xml():
    # The _xml() gate is what guarantees well-formed XML; make sure it actually fails closed.
    from xml.etree.ElementTree import ParseError

    from idegym.plugins.plugin_utils import ide_config

    with raises(ParseError):
        ide_config._xml("<application><unclosed></application>")
...
def test_idea_render_installs_open_project_plugin_when_project_in_ctx():
    ...
    assert "chmod +x /usr/local/bin/check-mcp /usr/local/bin/start-ide" in fragment
    assert "/usr/local/bin/start-ide.sh" not in fragment
...
def test_plugin_base_get_mcp_upstream_default_is_none():
    ctx = BuildContext(base="debian:bookworm-slim")
    assert PluginBase().get_mcp_upstream(ctx) is None
...
def test_to_spec_auto_emits_mcp_config_for_plugin_with_mcp_upstream():
    ...
    image = (
        Image.from_base("debian:bookworm-slim").with_plugin(Project.from_local("test-project")).with_plugin(PyCharm())
    )
    spec = image.to_spec()
...
def test_mcp_fragment_no_user_switch_when_root():
    ...
    ctx = BuildContext(base="debian:bookworm-slim", current_user="root").with_extra("idegym.has_project", True)
...
def test_mcp_fragment_wraps_with_user_switch_for_non_root():
    ...
    ctx = BuildContext(base="debian:bookworm-slim", current_user="appuser").with_extra("idegym.has_project", True)
    fragment = _mcp_upstream_fragment(PyCharm(), ctx)
    assert "mcp-upstreams.d/pycharm.json" in fragment
...
def test_to_spec_raises_for_unregistered_plugin_with_unsafe_class_name():
    ...
    class UnsafeNamePlugin_With_Underscores(PluginBase):
        def get_mcp_upstream(self, ctx: BuildContext) -> str:
...
def test_to_spec_pycharm_before_idegym_server_writes_pycharm_to_plugins_json():
    ...
    image = (
        Image.from_base("debian:bookworm-slim")
        .with_plugin(Project.from_local("test-project"))
        .with_plugin(PyCharm())
...
def test_shared_supervisord_references_bare_start_command():
    # One supervisord program for both IDEs; IdeGYMServer renames /usr/local/bin/*.{py,sh} to bare
    # hyphenated commands, so the program command must reference the bare name, not a .sh filename.
    supervisord = (_REPO_ROOT / "plugins" / "plugin-utils" / "scripts" / "supervisord-ide.conf").read_text()
    assert "command=/usr/local/bin/start-ide\n" in supervisord
    assert "start-ide.sh" not in supervisord


def test_shared_start_ide_script_sources_check_mcp_by_bare_name():
    # The single shared entrypoint (installed as start-ide) sources check-mcp by its bare name so the
    # source resolves after the /usr/local/bin/*.sh rename pass.
    start = (_REPO_ROOT / "plugins" / "plugin-utils" / "scripts" / "start-ide.sh").read_text()
