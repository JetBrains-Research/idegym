...
> open-project zip) declare those files via `get_context_files()`. The build driver stages them into a
> temporary build context automatically, so the build works from any working directory — you do **not**
> need a checkout of the idegym repo. See
