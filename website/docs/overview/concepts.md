...
```mermaid
flowchart LR
    client["Client<br/>(your code / an agent)"]
    orch["Orchestrator"]
    image["Image<br/>(built from plugins)"]
    server["Server pod<br/>(sandboxed environment)"]
    reward["Rewards & tools"]
    watcher["Watcher"]
    ...
    click reward "/idegym/architecture/rewards-tools" "Rewards & tools"
