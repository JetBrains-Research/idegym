...
- **Local Docker build** stages any missing declared files in place into the caller's build
  context (removing only what it created afterwards), so `COPY` resolves regardless of the
  caller's working directory while their `.dockerignore` and sibling `COPY` sources stay in effect.
...
plugins' [`plugins/pyproject.toml`](https://github.com/JetBrains-Research/idegym/blob/main/plugins/pyproject.toml)); `plugin_asset()` also falls back
